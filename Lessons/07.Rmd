...
pH, nutrient level, DO, turbidity, heavy metals, ions
...
```{r other_ion_data}
Nitrate.raw <- readNWISqw('03201300', parameterCd = '71851')
Nitrate.dat <- Nitrate.raw %>%
 select(Date = sample_dt, Nitrate_ugL = result_va)
  
 
Nitrate.plot <- ggplot(Nitrate.dat, aes(x = Date, y = Nitrate_ugL)) +
  geom_point() +
  geom_vline(xintercept = 1977, color = "grey", alpha = 0.8, lwd = 2, lty = 2) + 
  labs(y = expression("Nitrate ("*mu*g*l^-1*")")) +
  ggtitle("Nitrate Concentrations in the Kanawha River")
print(Nitrate.plot)
```
