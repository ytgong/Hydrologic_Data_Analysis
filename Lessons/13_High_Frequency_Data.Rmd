...
> flashflood, spill of contaminants, big daily changes (temperature, oxygen)
...
dischargedatTFC2018 <- readNWISuv( #uv->instantaneous data
...
> overland flow (pipes, "normal" olf), groundwater, directly into river channel
...
Export_EC$BaseflowExport_cf/Export_EC$TotalExport_cf
...
> Ellerbe Creek is flashier (it has a higher RBI)

```{r}
dischargedatIL2018 <- readNWISuv( #uv->instantaneous data
  site = "05539900", #Third Fork Creek at Woodcroft Parkway Near Blands
  parameterCd = "00060",
  startDate = "2018-01-01", 
  endDate = "2019-01-01"
  ) %>% 
  renameNWISColumns()
ILbaseflow <- BaseflowSeparation(
  dischargedatIL2018$Flow_Inst, 
  filter_parameter = 0.925, 
  passes = 3
  )

IL2018 <- cbind(dischargedatIL2018, ILbaseflow)
