...
eutrophication (nutrients) -> dissolved Oxygen -> microbes
invasive species
contaminants
...
LAGOSnutrient <- LAGOSdata$epi_nutr #epi means Epilimnion - where data is easiest to collect
# on average more than 1 observation per lake
...
LAGOStrophic <- 
  ...
  drop_na(chla:secchi) # move an entire row when there's a NA
...
print(chlbymonth)
# higher in summer: more light, higher temperature
...
ggplot(LAGOStrophic, 
  ...
  scale_color_viridis_c(option = "magma")
print(secchibymonth) #more concentrated than chloro
...
> Jitter: different states/lakes have different baselines so tricky to say what level is "good" - but if overall trend is going to eutriphication then it's probably bad.
