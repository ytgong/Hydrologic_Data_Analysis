...
# because otherwise it will return the min, max, mean, sd, etc. of "depth" which don't really make sense
...
TempvsDepth <- 
  ...
  labs(y = expression("Temperature "(degree*C)), x = "Depth (m)") # a good way to make labels for axises
print(TempvsDepth)
```

May to August data.
Weird points: November
...
Here are are highlighting depths considered the **metalimnion** or **thermocline**. rapid temp decrease
```{r}
...
# dotted line: 4 degrees. max density. almost as cold as a lake can get.
...
wind + small density gradient => lake mixes, temperature passes down depth
...
1. 4 degree
2. epilimnion: surface layer
   hypolimnion: bottom, stagnnant
   metalimnion / thermocline: temp decreases rapidly
3. wind makes lake mix!